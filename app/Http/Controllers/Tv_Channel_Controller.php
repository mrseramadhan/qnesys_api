...
use App\Format_API;
use App\Ms_Tv_Channel;
use Exception;

class Tv_Channel_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Tv_Channel::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_tv_channel WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_tv_channel WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        ...
        $move_logo = 'Tv_Channel_'.date('dmYHis').'.'.$logo->getClientOriginalName();

        $logo->move($path,$move_logo);

        if($request_body->id_tv_channel == null){
            try{
                $result=Ms_Tv_Channel::create([
                  ...
                  if($result->id_tv_channel)
                  {
                      $data_out=(object)
                        array(
                        'id_tv_channel'=>$result->id_tv_channel
                      );
                      $controller_success++;
                      $controller_message='Success to create new tv channel';
                  }
                  else
                  {
                      $controller_failed++;
                      $controller_message='Failed to create new tv channel';
                  }
            }
            catch(\Illuminate\Database\QueryException $e)
            {
            ...
            $result=Ms_Tv_Channel::where('id_tv_channel','=',$request_body->id_tv_channel)
                ->update([
                    'id_tv_category' => $request_body->id_tv_category,
                    'channel_name'   => $request_body->channel_name,
                    'description'    => $request_body->description,
              ...
              if($result)
              {
                  ...
                  $controller_success++;
                  $controller_message='Success to update tv channel';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to update tv channel';
              }
        }
      }
      else
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        ...
        $data_tv_channel = Ms_Tv_Channel::select('logo')->where('id_tv_channel', $request_body->id_tv_channel)->first();
        if($data_tv_channel){
          $filepath = 'assets/img/';
          $logo = $filepath.$data_tv_channel->logo;
          @unlink($logo);
        }else{
          $controller_failed++;
        ...
        if(!is_array($request_body->id_tv_channel))
      	{
        	$request_body->id_tv_channel=array($request_body->id_tv_channel);
          if(Ms_Tv_Channel::whereIn('id_tv_channel',$request_body->id_tv_channel)->delete())
          {
            $data_out=$request_body->id_tv_channel;
            if(is_array($request_body->id_tv_channel))
            {
              $controller_success+=count($request_body->id_tv_channel);
            }
            else
            {
              $controller_success++;
            ...
            $controller_message='Success to delete tv channel';
          }
          else
          {
            $controller_failed++;
