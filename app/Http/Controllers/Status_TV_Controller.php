...
use App\Format_API;
use App\Tr_Status_TV;

class Status_TV_Controller extends Controller
{
  function read(Request $request)
  {
    ...
    if($check_result->accept)
    {
        ...
        $query=Tr_Status_TV::select($select);
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from tr_status_tv WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from tr_status_tv WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(empty(@$request_body->id_status_tv))
      {
        ...
        if(!empty($result->id_status_tv))
        {
          $data_out=(object)
            array(
              'id_status_tv'=>$result->id_status_tv
          );
          $controller_success++;
          $controller_message='Success to create new status tv ';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new status tv';
        }
      }
      else
      {
        $data_update=Tr_Status_TV::find($request_body->id_status_tv);
        if(!empty($data_update))
        {
          ...
          if($execute)
          {
            $data_out=(object)
              array(
                'id_status_tv'=>$request_body->id_status_tv
            );
            $controller_success++;
            $controller_message='Success to update status tv';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update status tv';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_status_tv))
      {
        $request_body->id_status_tv=array($request_body->id_status_tv);
      }

      if(Tr_Status_TV::whereIn('id_status_tv',$request_body->id_status_tv)->delete())
      {
        $data_out=$request_body->id_status_tv;
        if(is_array($request_body->id_status_tv))
        {
          $controller_success+=count($request_body->id_status_tv);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete status tv';
      }
      else
      {
        $controller_failed++;
