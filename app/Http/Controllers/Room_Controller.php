...
class Room_Controller extends Controller
{
    ...
    if($check_result->accept)
    {
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from ms_room WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from ms_room WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(empty(@$request_body->id_room))
      {
        ...
        try{
          $result=Ms_Room::create([
            'id_room_category'=>@$request_body->id_room_category,
            'no_room'=>@$request_body->no_room,
        ...
        if(!empty($result->id_room))
        {
          ...
          $controller_success++;
          $controller_message='Success to create new room';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new room';
        }
      }
      else
      {
        $data_update=Ms_Room::find($request_body->id_room);
        if(!empty($data_update))
        {
          ...
          if(!empty($request_body->no_room))
          {
            $data_update->no_room = $request_body->no_room;
          }
          if(!empty($request_body->status))
          {
            $data_update->status = $request_body->status;
          }
          if(!empty($request_body->lantai))
          {
            $data_update->lantai = $request_body->lantai;
          }
          ...
          if(@$execute)
          {
            ...
            $controller_success++;
            $controller_message='Success to update room data';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update room data';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_room))
      {
        $request_body->id_room=array($request_body->id_room);
      }

      if(Ms_Room::whereIn('id_room',$request_body->id_room)->delete())
      {
        $data_out=$request_body->id_room;
        if(is_array($request_body->id_room))
        {
          $controller_success+=count($request_body->id_room);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete room';
      }
      else
      {
        $controller_failed++;
