...
use App\Format_API;
use App\Tr_Division_Privilege_Menu;
use Exception;

class Division_Privilege_Menu_Controller extends Controller
{
  function read(Request $request)
  {
    ...
    if($check_result->accept)
    {
        ...
        $query=Tr_Division_Privilege_Menu::select($select);
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from tr_division_privilege_menu WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from tr_division_privilege_menu WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
    ...
    return response(
      set_format_api(
        @$data_out,
        array(
          'primary'=>'id_division_privilege_menu',
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(empty(@$request_body->id_division_privilege_menu))
      {
        try{
          $result=Tr_Division_Privilege_Menu::create([
            'id_menu'=>@$request_body->id_menu,
        ...
        if(!empty($result->id_division_privilege_menu))
        {
          $data_out=(object)
            array(
              'id_division_privilege_menu'=>$result->id_division_privilege_menu
          );
          $controller_success++;
          $controller_message.='Success to create new division privilege menu';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new division privilege menu';
        }
      }
      else
      {
        $data_update=Tr_Division_Privilege_Menu::find($request_body->id_division_privilege_menu);
        if(!empty($data_update))
        {
          if(!empty($request_body->id_menu))
          {
            $data_update->id_menu = $request_body->id_menu;
          }
          if(!empty($request_body->id_division_privilege))
          {
            $data_update->id_division_privilege = $request_body->id_division_privilege;
          }
          ...
          if(@$execute)
          {
            $data_out=(object)
              array(
                'id_division_privilege_menu'=>$request_body->id_division_privilege_menu
            );
            $controller_success++;
            $controller_message='Success to update division privilege menu';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update division privilege menu';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    //Untuk melakukan pengecheckan data dan hanya untuk IN
    $checker=array(
      'id_division_privilege_menu'=>true
    );
    $check_result=check_input_format($checker,$request_body);
    $controller_message='';
    $controller_failed=0;
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_division_privilege_menu))
      {
        $request_body->id_division_privilege_menu=array($request_body->id_division_privilege_menu);
      }

      if(Tr_Division_Privilege_Menu::whereIn('id_division_privilege_menu',$request_body->id_division_privilege_menu)->delete())
      {
        $data_out=$request_body->id_division_privilege_menu;
        if(is_array($request_body->id_division_privilege_menu))
        {
          $controller_success+=count($request_body->id_division_privilege_menu);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete division privilege menu';
      }
      else
      {
        $controller_failed++;
