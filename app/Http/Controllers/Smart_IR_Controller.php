...
class Smart_IR_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Dt_Smart_IR::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from dt_smart_ir WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from dt_smart_ir WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(empty(@$request_body->id_smart_ir))
        {
          try{
            $result=Dt_Smart_IR::create([
              'id_room'=>@$request_body->id_room,
              'type_koneksi'=>@$request_body->koneksi,
          ...
          if(!empty($result->id_smart_ir))
          {
            $data_out=(object)
              array(
                'id_smart_ir'=>$result->id_smart_ir
            );
            $controller_success++;
            $controller_message='Success to create new smart ir';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to create new smart ir';
          }
        }
        else
        {
          $data_update=Dt_Smart_IR::find($request_body->id_smart_ir);
          if(!empty($data_update))
          {
            if(!empty($request_body->id_smart_ir))
            {
              $data_update->id_smart_ir = $request_body->id_smart_ir;
            }
            ...
            if($execute)
            {
              $data_out=(object)
                array(
                  'id_smart_ir'=>$request_body->id_smart_ir
              );
              $controller_success++;
              $controller_message='Success to update smart ir data';
            }
            else
            {
              $controller_failed++;
              $controller_message.=', Failed to update smart ir data';
            }
          }
          else
          {
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(!is_array($request_body->id_smart_ir))
        {
          $request_body->id_smart_ir=array($request_body->id_smart_ir);
        }
  
        if(Dt_Smart_IR::whereIn('id_smart_ir',$request_body->id_smart_ir)->delete())
        {
          $data_out=$request_body->id_smart_ir;
          if(is_array($request_body->id_smart_ir))
          {
            $controller_success+=count($request_body->id_smart_ir);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete smart ir';
        }
        else
        {
          $controller_failed++;
