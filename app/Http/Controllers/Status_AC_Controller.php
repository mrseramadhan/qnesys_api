...
use App\Format_API;
use App\Tr_Status_AC;

class Status_AC_Controller extends Controller
{
  function read(Request $request)
  {
    ...
    if($check_result->accept)
    {
        ...
        $query=Tr_Status_AC::select($select);
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from tr_status_ac WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from tr_status_ac WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(empty(@$request_body->id_status_ac))
      {
        ...
        if(!empty($result->id_status_ac))
        {
          $data_out=(object)
            array(
              'id_status_ac'=>$result->id_status_ac
          );
          $controller_success++;
          $controller_message='Success to create new status ac ';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new status ac';
        }
      }
      else
      {
        $data_update=Tr_Status_AC::find($request_body->id_status_ac);
        if(!empty($data_update))
        {
          ...
          if($execute)
          {
            $data_out=(object)
              array(
                'id_status_ac'=>$request_body->id_status_ac
            );
            $controller_success++;
            $controller_message='Success to update status ac';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update status ac';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_status_ac))
      {
        $request_body->id_status_ac=array($request_body->id_status_ac);
      }

      if(Tr_Status_AC::whereIn('id_status_ac',$request_body->id_status_ac)->delete())
      {
        $data_out=$request_body->id_status_ac;
        if(is_array($request_body->id_status_ac))
        {
          $controller_success+=count($request_body->id_status_ac);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete status ac';
      }
      else
      {
        $controller_failed++;
