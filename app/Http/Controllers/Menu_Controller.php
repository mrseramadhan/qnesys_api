...
class Menu_Controller extends Controller
{
    ...
    if($check_result->accept)
    {
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from ms_menu WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from ms_menu WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      ...
      {
        try{
          $result=Ms_Menu::create([
            'menu_name'=>@$request_body->menu_name,
            'id_menu_parent'=>@$request_body->id_menu_parent,
        ...
        if(!empty($result->id_menu))
        {
          ...
          $controller_success++;
          $controller_message.='Success to create new menu';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new menu';
        }
      }
      else
      {
        $data_update=Ms_Menu::find($request_body->id_menu);
        if(!empty($data_update))
        {
          if(!empty($request_body->menu_name))
          {
            $data_update->menu_name = $request_body->menu_name;
          }
          if(!empty($request_body->id_menu_parent))
          {
            $data_update->id_menu_parent = $request_body->id_menu_parent;
          }
          ...
          if(@$execute)
          {
            ...
            $controller_success++;
            $controller_message='Success to update menu';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update menu';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_menu))
      {
        $request_body->id_menu=array($request_body->id_menu);
      }

      if(Ms_Menu::whereIn('id_menu',$request_body->id_menu)->delete())
      {
        $data_out=$request_body->id_menu;
        if(is_array($request_body->id_menu))
        {
          $controller_success+=count($request_body->id_menu);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete menu';
      }
      else
      {
        $controller_failed++;
