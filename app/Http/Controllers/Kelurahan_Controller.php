...
use app\Format_API;
use App\Ms_Kelurahan;
use Exception;

class Kelurahan_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Kelurahan::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_kelurahan WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_kelurahan WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if($request_body->id_kelurahan == null){
            $result=Ms_Kelurahan::create([
                'id_kabupaten'  => $request_body->id_kabupaten,
                'kelurahan'     => $request_body->kelurahan,
              ]);

              if($result->id_kelurahan)
              {
                  $data_out=(object)
                    array(
                  ...
                  $controller_success++;
                  $controller_message='Success to create new kelurahan';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to create new kelurahan';
              }
        }else{
            $result=Ms_Kelurahan::where('id_kelurahan','=',$request_body->id_kelurahan)
                ->update([
                    'id_kabupaten'  => $request_body->id_kabupaten,
                    'kelurahan'  => $request_body->kelurahan,
              ]);
              ...
              if($result)
              {
                  $data_out=(object)
                    array(
                    'id_kelurahan'=>$request_body->id_kelurahan
                  );
                  $controller_success++;
                  $controller_message='Success to update kelurahan';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to update kelurahan';
              }
        }
      }
      else
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(!is_array($request_body->id_kelurahan))
      	{
        	$request_body->id_kelurahan=array($request_body->id_kelurahan);
        }
        if(Ms_Kelurahan::whereIn('id_kelurahan',$request_body->id_kelurahan)->delete())
        {
          $data_out=$request_body->id_kelurahan;
          if(is_array($request_body->id_kelurahan))
          {
            $controller_success+=count($request_body->id_kelurahan);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete kelurahan';
        }
        else
        {
          $controller_failed++;
