...
use app\Format_API;
use App\Ms_Facilities_Category;
use Exception;

class Facilities_Category_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Facilities_Category::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_facilities_category WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_facilities_category WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
      ...
      return response(
        set_format_api(
          @$data_out,
          array(
            'primary'=>'id_facilities_category',
