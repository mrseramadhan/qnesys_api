      ...
      if($check_result->accept)
      {
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from Ms_Hotel WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from Ms_Hotel WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
        ...
        if($request_body->id_hotel == null){
            $result=Ms_Hotel::create([
              ...
              if($result->id_hotel)
              {
                  ...
                  $controller_success++;
                  $controller_message='Success to create new hotel';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to create new hotel';
              }
        }else{
            $result=Ms_Hotel::where('id_hotel','=',$request_body->id_hotel)
                ->update([
                'id_corporation'      => $request_body->id_corporation,
                'hotel_name'          => $request_body->hotel_name,
                'hotel_address'       => $request_body->hotel_address,
