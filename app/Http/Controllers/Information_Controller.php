...
class Information_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Information::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_information WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_information WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
