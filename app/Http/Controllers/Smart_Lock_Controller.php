...
use App\Format_API;
use App\Dt_Smart_Lock;

class Smart_Lock_Controller extends Controller
{
    function read(Request $request)
  {
    ...
    if($check_result->accept)
    {
        ...
        $query=Dt_Smart_Lock::select($select);
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from dt_smart_lock WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from dt_smart_lock WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(empty(@$request_body->id_smart_lock))
      {
        try{
          $result=Dt_Smart_Lock::create([
        ...
        if(!empty($result->id_smart_lock))
        {
          $data_out=(object)
            array(
              'id_smart_lock'=>$result->id_smart_lock
          );
          $controller_success++;
          $controller_message='Success to create new smart lock';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new smart lock';
        }
      }
      else
      {
        $data_update=Dt_Smart_Lock::find($request_body->id_smart_lock);
        if(!empty($data_update))
        {
          ...
          {
            $data_update->mac_address = $request_body->mac_address;
          }
          ...
          if($execute)
          {
            $data_out=(object)
              array(
                'id_smart_lock'=>$request_body->id_smart_lock
            );
            $controller_success++;
            $controller_message='Success to update smart lock data';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update smart lock data';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_smart_lock))
      {
        $request_body->id_smart_lock=array($request_body->id_smart_lock);
      }

      if(Dt_Smart_Lock::whereIn('id_smart_lock',$request_body->id_smart_lock)->delete())
      {
        $data_out=$request_body->id_smart_lock;
        if(is_array($request_body->id_smart_lock))
        {
          $controller_success+=count($request_body->id_smart_lock);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete smart lock';
      }
      else
      {
        $controller_failed++;
