...
class Provinsi_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Provinsi::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_provinsi WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_provinsi WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if($request_body->id_provinsi == null){
            $result=Ms_Provinsi::create([
                'provinsi'  => $request_body->provinsi,
              ]);

              if($result->id_provinsi)
              {
                  $data_out=(object)
                    array(
                  ...
                  $controller_success++;
                  $controller_message='Success to create new provinsi';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to create new provinsi';
              }
        }else{
            $result=Ms_Provinsi::where('id_provinsi','=',$request_body->id_provinsi)
                ->update([
                    'provinsi'  => $request_body->provinsi,
              ]);
              ...
              if($result)
              {
                  ...
                  $controller_success++;
                  $controller_message='Success to update provinsi';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to update provinsi';
              }
        }
      }
      else
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(!is_array($request_body->id_provinsi))
      	{
        	$request_body->id_provinsi=array($request_body->id_provinsi);
        }
        if(Ms_Provinsi::whereIn('id_provinsi',$request_body->id_provinsi)->delete())
        {
          $data_out=$request_body->id_provinsi;
          if(is_array($request_body->id_provinsi))
          {
            $controller_success+=count($request_body->id_provinsi);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete provinsi';
        }
        else
        {
          $controller_failed++;
