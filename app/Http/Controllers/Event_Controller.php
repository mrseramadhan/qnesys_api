      ...
      if($check_result->accept)
      {
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_event WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_event WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(Ms_Event::whereIn('id_event',$request_body->id_event)->delete())
        {
          ...
          {
            $controller_success+=count($request_body->id_event);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete event hotel';
        }
        else
        {
          $controller_failed++;
