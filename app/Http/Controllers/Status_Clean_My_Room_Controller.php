...
use App\Format_API;
use App\Tr_Status_Clean_My_Room;

class Status_Clean_My_Room_Controller extends Controller
{
  function read(Request $request)
  {
    ...
    if($check_result->accept)
    {
        ...
        $query=Tr_Status_Clean_My_Room::select($select);
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from tr_status_my_room WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from tr_status_my_room WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
    ...
    return response(
      set_format_api(
        @$data_out,
        array(
          'primary'=>'id_status_clean_my_room',
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(empty(@$request_body->id_status_clean_my_room))
      {
        ...
        try{
          $result=Tr_Status_Clean_My_Room::create([
        ...
        if(!empty($result->id_status_clean_my_room))
        {
          $data_out=(object)
            array(
              'id_status_clean_my_room'=>$result->id_status_clean_my_room
          );
          $controller_success++;
          $controller_message='Success to create new status clean my room ';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new status clean my room';
        }
      }
      else
      {
        $data_update=Tr_Status_Clean_My_Room::find($request_body->id_status_clean_my_room);
        if(!empty($data_update))
        {
          ...
          if($execute)
          {
            $data_out=(object)
              array(
                'id_status_clean_my_room'=>$request_body->id_status_clean_my_room
            );
            $controller_success++;
            $controller_message='Success to update status clean my room';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update status clean my room';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_status_clean_my_room))
      {
        $request_body->id_status_clean_my_room=array($request_body->id_status_clean_my_room);
      }

      if(Tr_Status_Clean_My_Room::whereIn('id_status_clean_my_room',$request_body->id_status_clean_my_room)->delete())
      {
        $data_out=$request_body->id_status_clean_my_room;
        if(is_array($request_body->id_status_clean_my_room))
        {
          $controller_success+=count($request_body->id_status_clean_my_room);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete status clean my room';
      }
      else
      {
        $controller_failed++;
