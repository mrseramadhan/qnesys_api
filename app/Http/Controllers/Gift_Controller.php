...
class Gift_Controller extends Controller
{
      ...
      if($check_result->accept)
      {
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_gift WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_gift WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        ...
        $path = 'assets/img';
        $move_picture = 'Gift_'.date('dmYHis').'.'.$picture->getClientOriginalName();
        $picture->move($path,$move_picture);

        if($request_body->id_gift == null){
          try{
            $result=Ms_Gift::create([
              'gift_name'       => $request_body->gift_name,
              'detail'          => $request_body->detail,
              'picture'         => $move_picture,
              'start_periode'   => $request_body->start_periode,
              'end_periode'     => $request_body->end_periode,
            ]);
            ...
            if($result->id_gift)
            {
                $data_out=(object)
                  array(
                  'id_gift'=>$result->id_gift
                );
                $controller_success++;
                $controller_message='Success to create new Gift';
            }
            else
            {
                $controller_failed++;
                $controller_message='Failed to create new Gift';
            }
          }
          catch(\Illuminate\Database\QueryException $e)
          {
            ...
            $result=Ms_Gift::where('id_gift','=',$request_body->id_gift)
                ->update([
                    'gift_name'       => $request_body->gift_name,
                    'detail'          => $request_body->detail,
                    'picture'         => $move_picture,
              ...
              if($result)
              {
                  $data_out=(object)
                    array(
                    'id_gift'=>$request_body->id_gift
                  );
                  $controller_success++;
                  $controller_message='Success to update Gift';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to update Gift';
              }
        }
      }
      else
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        $data_gift = Ms_Gift::select('picture')->where('id_gift', $request_body->id_gift)->first();
        if($data_gift){
          $filepath = 'assets/img/';
          $picture = $filepath.$data_gift->picture;
          @unlink($picture);
        }else{
          $controller_failed++;
        ...
        if(!is_array($request_body->id_gift))
      	{
          $request_body->id_gift=array($request_body->id_gift);
        }
        if(Ms_Gift::whereIn('id_gift',$request_body->id_gift)->delete())
        {
          $data_out=$request_body->id_gift;
          if(is_array($request_body->id_gift))
          {
            $controller_success+=count($request_body->id_gift);
          }
          else
          {
            $controller_success++;
