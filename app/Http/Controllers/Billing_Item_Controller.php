...
use App\Format_API;
use App\Dt_Billing_Item;
use Exception;

class Billing_Item_Controller extends Controller
{
  function read(Request $request)
  {
    ...
    if($check_result->accept)
    {
        ...
        $query=Dt_Billing_Item::select($select);
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from tr_billing_item WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from tr_billing_item WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
