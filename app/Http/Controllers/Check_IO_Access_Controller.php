...
use App\Format_API;
use App\Ms_Guest;
use App\Dt_Check_IO_Access;
use File;

class Check_IO_Access_Controller extends Controller
{
      ...
      if($check_result->accept)
      {
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from dt_check_io_access WHERE '.$request_body->custom_condition);
              }
              else
              {
      ...
      if($check_result->accept)
      {
        if(empty(@$request_body->id_check_io_access))
        {
          ...
          if(!empty($result->id_check_io_access))
          {
            $data_out=(object)
              array(
                'id_check_io_access'=>$result->id_check_io_access
            );
            $controller_success++;
            $controller_message='Success to create new room access';
          }
          else
          {
            $controller_failed++;
        ...
        {
          $data_update=Dt_Check_IO_Access::find($request_body->id_check_io_access);
          if(!empty($data_update))
          {
            ...
            if($execute)
            {
              $data_out=(object)
                array(
                  'id_check_io_access'=>$request_body->id_check_io_access
              );
              $controller_success++;
              $controller_message='Success to update room access';
            }
            else
            {
              $controller_failed++;
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(!is_array($request_body->id_check_io_access))
        {
          $request_body->id_check_io_access=array($request_body->id_check_io_access);
        }

        $file_pic=Dt_Check_IO_Access::whereIn('id_check_io_access',$request_body->id_check_io_access)->get();

        if(Dt_Check_IO_Access::whereIn('id_check_io_access',$request_body->id_check_io_access)->delete())
        {
          $data_out=$request_body->id_check_io_access;
          if(is_array($request_body->id_check_io_access))
          {
            $controller_success+=count($request_body->id_check_io_access);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete room access';
        }
        else
        {
          $controller_failed++;
