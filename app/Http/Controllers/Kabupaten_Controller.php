...
class Kabupaten_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Kabupaten::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_kabupaten WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_kabupaten WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if($request_body->id_kabupaten == null){
            $result=Ms_Kabupaten::create([
                'id_provinsi'  => $request_body->id_provinsi,
                'kabupaten'  => $request_body->kabupaten,
              ]);

              if($result->id_kabupaten)
              {
                  $data_out=(object)
                    array(
                  ...
                  $controller_success++;
                  $controller_message='Success to create new kabupaten';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to create new kabupaten';
              }
        }else{
            $result=Ms_Kabupaten::where('id_kabupaten','=',$request_body->id_kabupaten)
                ->update([
                    'id_provinsi'  => $request_body->id_provinsi,
                    'kabupaten'  => $request_body->kabupaten,
              ]);
              ...
              if($result)
              {
                  ...
                  $controller_success++;
                  $controller_message='Success to update kabupaten';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to update kabupaten';
              }
        }
      }
      else
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(!is_array($request_body->id_kabupaten))
      	{
        	$request_body->id_kabupaten=array($request_body->id_kabupaten);
        }
        if(Ms_Kabupaten::whereIn('id_kabupaten',$request_body->id_kabupaten)->delete())
        {
          $data_out=$request_body->id_kabupaten;
          if(is_array($request_body->id_kabupaten))
          {
            $controller_success+=count($request_body->id_kabupaten);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete kabupaten';
        }
        else
        {
          $controller_failed++;
