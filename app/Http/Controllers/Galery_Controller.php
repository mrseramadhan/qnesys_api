...
class Galery_Controller extends Controller
{
      ...
      if($check_result->accept)
      {
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_galery WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_galery WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(Ms_Galery::whereIn('id_galery',$request_body->id_galery)->delete())
        {
          ...
          {
            $controller_success+=count($request_body->id_galery);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete galery';
        }
        else
        {
          $controller_failed++;
