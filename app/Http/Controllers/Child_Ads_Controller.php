...
use app\Format_API;
use App\Ms_Child_Ads;
use Exception;

class Child_Ads_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Child_Ads::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_child_ads WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_child_ads WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        ...
        $path = 'assets/img';
        $move_file = 'CHILDADS_'.date('dmYHis').'.'.$file->getClientOriginalName();

        $file->move($path,$move_file);

        if($request_body->id_child_ads == null){
          try{
            $result=Ms_Child_Ads::create([
              ...
              if($result->id_child_ads)
              {
                  $data_out=(object)
                    array(
                    'id_child_ads'=>$result->id_child_ads
                  );
                  $controller_success++;
                  $controller_message='Success to create new child ads';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to create new child ads';
              }
            }
              catch(\Illuminate\Database\QueryException $e)
            {
            ...
            $result=Ms_Child_Ads::where('id_child_ads','=',$request_body->id_child_ads)
                ->update([
                  'id_master_ads'=> $request_body->id_master_ads,
                  'id_hotel'     => $request_body->id_hotel,
                  'nama_produk'  => $request_body->nama_produk,
              ...
              if($result)
              {
                  $data_out=(object)
                    array(
                    'id_child_ads'=>$request_body->id_child_ads
                  );
                  $controller_success++;
                  $controller_message='Success to update child ads';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to update child ads';
              }
        }
      }
      else
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        // $data_around = Ms_Child_Ads::select('picture')->where('id_master_ads', $request_body->id_master_ads)->first();
        // if($data_around){
        //   $filepath = 'assets/img/';
        ...
        if(!is_array($request_body->id_child_ads))
      	{
          $request_body->id_child_ads=array($request_body->id_child_ads);
        }
        if(Ms_Child_Ads::whereIn('id_child_ads',$request_body->id_child_ads)->delete())
        {
          $data_out=$request_body->id_child_ads;
          if(is_array($request_body->id_child_ads))
          {
            $controller_success+=count($request_body->id_child_ads);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete child ads';
        }
        else
        {
          $controller_failed++;
