...
class User_Controller extends Controller
{
      ...
      if($check_result->accept)
      {
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_user WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_user WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function update(Request $request)
    {
      ...
      if($check_result->accept)
      {
        $data_update=Ms_User::find($request_body->id_user);
        if(!empty($data_update))
        {
          ...
          if($execute)
          {
            ...
            $controller_success++;
            $controller_message='Success to update user';
          }
          else
          {
            $controller_failed++;
            $controller_message='Failed to update user';
          }
        }
        else
        {
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(Ms_User::whereIn('id_user',$request_body->id_user)->delete())
        {
          ...
          {
            $controller_success+=count($request_body->id_user);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete user';
        }
        else
        {
          $controller_failed++;
