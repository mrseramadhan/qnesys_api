...
use App\Format_API;
use App\Tr_Status_DND;

class Status_DND_Controller extends Controller
{
  function read(Request $request)
  {
    ...
    if($check_result->accept)
    {
        ...
        $query=Tr_Status_DND::select($select);
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from tr_status_dnd WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from tr_status_dnd WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(empty(@$request_body->id_status_dnd))
      {
        ...
        if(!empty($result->id_status_dnd))
        {
          $data_out=(object)
            array(
              'id_status_dnd'=>$result->id_status_dnd
          );
          $controller_success++;
          $controller_message='Success to create new status dnd ';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new status dnd';
        }
      }
      else
      {
        $data_update=Tr_Status_DND::find($request_body->id_status_dnd);
        if(!empty($data_update))
        {
          ...
          if($execute)
          {
            $data_out=(object)
              array(
                'id_status_dnd'=>$request_body->id_status_dnd
            );
            $controller_success++;
            $controller_message='Success to update status dnd';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update status dnd';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_status_dnd))
      {
        $request_body->id_status_dnd=array($request_body->id_status_dnd);
      }

      if(Tr_Status_DND::whereIn('id_status_dnd',$request_body->id_status_dnd)->delete())
      {
        $data_out=$request_body->id_status_dnd;
        if(is_array($request_body->id_status_dnd))
        {
          $controller_success+=count($request_body->id_status_dnd);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete status dnd';
      }
      else
      {
        $controller_failed++;
