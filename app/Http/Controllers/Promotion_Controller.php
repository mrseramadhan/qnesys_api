...
class Promotion_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Promotion::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_promotion WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_promotion WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        ...
        $path = 'assets/img';
        $move_picture = 'Promotion_'.date('dmYHis').'.'.$picture->getClientOriginalName();
        $picture->move($path,$move_picture);

        if($request_body->id_promotion == null){
          try{
            $result=Ms_Promotion::create([
              'promotion_name'      => $request_body->promotion_name,
            ...
            if($result->id_promotion)
            {
                ...
                $controller_success++;
                $controller_message='Success to create new Promotion';
            }
            else
            {
                $controller_failed++;
                $controller_message='Failed to create new Promotion';
            }
          }
          catch(\Illuminate\Database\QueryException $e)
          {
            ...
            $result=Ms_Promotion::where('id_promotion','=',$request_body->id_promotion)
                ->update([
                    'promotion_name'      => $request_body->promotion_name,
                    'description'         => $request_body->description,
                    'picture'             => $move_picture,
              ...
              if($result)
              {
                  $data_out=(object)
                    array(
                    'id_promotion'=>$request_body->id_promotion
                  );
                  $controller_success++;
                  $controller_message='Success to update Promotion';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to update Promotion';
              }
        }
      }
      else
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        $data_promotion = Ms_Promotion::select('picture')->where('id_promotion', $request_body->id_promotion)->first();
        if($data_promotion){
          $filepath = 'assets/img/';
          $picture = $filepath.$data_promotion->picture;
          @unlink($picture);
        }else{
          $controller_failed++;
        ...
        if(!is_array($request_body->id_promotion))
      	{
          $request_body->id_promotion=array($request_body->id_promotion);
        }
        if(Ms_Promotion::whereIn('id_promotion',$request_body->id_promotion)->delete())
        {
          $data_out=$request_body->id_promotion;
          if(is_array($request_body->id_promotion))
          {
            $controller_success+=count($request_body->id_promotion);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete Gift';
        }
        else
        {
          $controller_failed++;
