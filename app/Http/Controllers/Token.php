      ...
      if($check_result->accept)
      {
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from sy_token WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from sy_token WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
