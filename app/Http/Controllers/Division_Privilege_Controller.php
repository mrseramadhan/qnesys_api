...
use App\Format_API;
use App\Ms_Division_Privilege;
use Exception;


class Division_Privilege_Controller extends Controller
{
  function read(Request $request)
  {
    ...
    if($check_result->accept)
    {
        ...
        $query=Ms_Division_Privilege::select($select);
        ...
        else
        {
          try{
            ...
            {
              $data_out=DB::select('SELECT '.$request_body->select.' from ms_division_privilege WHERE '.$request_body->custom_condition);
            }
            else
            {
              $data_out=DB::select('SELECT * from ms_division_privilege WHERE '.$request_body->custom_condition);
            }
            $controller_success++;
          }
          catch(\Illuminate\Database\QueryException $e)
  ...
  function create(Request $request)
  {
    ...
    if($check_result->accept)
    {
      ...
      {
        try{
          $result=Ms_Division_Privilege::create([
            'division_privilege_name'=>@$request_body->division_privilege_name
          ]);
        }
        catch(\Illuminate\Database\QueryException $e)
        {
        ...
        if(!empty($result->id_division_privilege))
        {
          $data_out=(object)
            array(
              'id_division_privilege'=>$result->id_division_privilege
          );
          $controller_success++;
          $controller_message.='Success to create new division privilege';
        }
        else
        {
          $controller_failed++;
          $controller_message.=', Failed to create new division privilege';
        }
      }
      else
      {
        $data_update=Ms_Division_Privilege::find($request_body->id_division_privilege);
        if(!empty($data_update))
        {
          if(!empty($request_body->division_privilege_name))
          {
            $data_update->division_privilege_name = $request_body->division_privilege_name;
          }
          ...
          if(@$execute)
          {
            ...
            $controller_success++;
            $controller_message='Success to update division privilege';
          }
          else
          {
            $controller_failed++;
            $controller_message.=', Failed to update division privilege';
          }
        }
        else
        {
  ...
  function delete(Request $request)
  {
    ...
    if($check_result->accept)
    {
      if(!is_array($request_body->id_division_privilege))
      {
        $request_body->id_division_privilege=array($request_body->id_division_privilege);
      }

      if(Ms_Division_Privilege::whereIn('id_division_privilege',$request_body->id_division_privilege)->delete())
      {
        $data_out=$request_body->id_division_privilege;
        if(is_array($request_body->id_division_privilege))
        {
          $controller_success+=count($request_body->id_division_privilege);
        }
        else
        {
          $controller_success++;
        ...
        $controller_message='Success to delete division privilege';
      }
      else
      {
        $controller_failed++;
