...
use App\Format_API;
use App\Ms_Public_Tv;
use Exception;

class Public_Tv_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Public_Tv::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_public_tv WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_public_tv WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        $pic_map = $request->file('picture');
        $path = 'assets/img';
        $move_pic_map = 'Public_Tv_'.$pic_map->getClientOriginalName();
        $pic_map->move($path,$move_pic_map);

        if($request_body->id_public_tv == null){
            try{
                $result=Ms_Public_Tv::create([
                    'id_hotel'     => $request_body->id_hotel,
                    'urutan'       => $request_body->urutan,
                    'detik'        => $request_body->detik,
                  ...
                  if($result->id_public_tv)
                  {
                      $data_out=(object)
                        array(
                        'id_public_tv'=>$result->id_public_tv
                      );
                      $controller_success++;
                      $controller_message='Success to create new public tv';
                  }
                  else
                  {
                      $controller_failed++;
                      $controller_message='Failed to create new public tv';
                  }
            }
            catch(\Illuminate\Database\QueryException $e)
            {
            ...
            $result=Ms_Public_Tv::where('id_public_tv','=',$request_body->id_public_tv)
                ->update([
                    'id_hotel'     => $request_body->id_hotel,
                    'urutan'       => $request_body->urutan,
                    'detik'        => $request_body->detik,
              ...
              if($result)
              {
                  $data_out=(object)
                    array(
                    'id_public_tv'=>$request_body->id_public_tv
                  );
                  $controller_success++;
                  $controller_message='Success to update public tv';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to update public tv';
              }
        }
      }
      else
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        $data_public_tv = Ms_Public_Tv::select('picture')->where('id_public_tv', $request_body->id_public_tv)->first();
        if($data_public_tv){
          $filepath = 'assets/img/';
          $picture = $filepath.$data_public_tv->picture;
          @unlink($picture);
        }else{
          $controller_failed++;
          $controller_message='Data not found';
        }
        if(!is_array($request_body->id_public_tv))
      	{
        	$request_body->id_public_tv=array($request_body->id_public_tv);
          if(Ms_Public_Tv::whereIn('id_public_tv',$request_body->id_public_tv)->delete())
          {
            $data_out=$request_body->id_public_tv;
            if(is_array($request_body->id_public_tv))
            {
              $controller_success+=count($request_body->id_public_tv);
            }
            else
            {
              $controller_success++;
            ...
            $controller_message='Success to delete public tv';
          }
          else
          {
            $controller_failed++;
