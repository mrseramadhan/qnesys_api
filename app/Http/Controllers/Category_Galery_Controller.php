...
use app\Format_API;
use App\Ms_Category_Galery;
use Exception;

class Category_Galery_Controller extends Controller
{
    function read(Request $request)
    {
      ...
      if($check_result->accept)
      {
          ...
          $query=Ms_Category_Galery::select($select);
          ...
          else
          {
            try{
              ...
              {
                $data_out=DB::select('SELECT '.$request_body->select.' from ms_category_galery WHERE '.$request_body->custom_condition);
              }
              else
              {
                $data_out=DB::select('SELECT * from ms_category_galery WHERE '.$request_body->custom_condition);
              }
              $controller_success++;
            }
            catch(\Illuminate\Database\QueryException $e)
    ...
    function create(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if($request_body->id_category_galery == null){
            $result=Ms_Category_Galery::create([
                'category_galery_name'  => $request_body->category_galery_name,
              ]);

              if($result->id_category_galery)
              {
                  $data_out=(object)
                    array(
                    'id_category_galery'=>$result->id_category_galery
                  );
                  $controller_success++;
                  $controller_message='Success to create new category galery';
              }
              else
              {
                  $controller_failed++;
                  $controller_message='Failed to create new category galery';
              }
        }else{
            $result=Ms_Category_Galery::where('id_category_galery','=',$request_body->id_category_galery)
                ->update([
                    'category_galery_name'  => $request_body->category_galery_name,
              ]);
    ...
    function delete(Request $request)
    {
      ...
      if($check_result->accept)
      {
        if(Ms_Category_Galery::whereIn('id_category_galery',$request_body->id_category_galery)->delete())
        {
          ...
          {
            $controller_success+=count($request_body->id_category_galery);
          }
          else
          {
            $controller_success++;
          ...
          $controller_message='Success to delete category galery';
        }
        else
        {
          $controller_failed++;
